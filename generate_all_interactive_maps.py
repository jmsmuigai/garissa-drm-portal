...
<body>
    ...
    <script>
        ...
        let cadastralData = {{ "type": "FeatureCollection", "features": [] }};
        ...
        let garissaCountyData = {{ "type": "FeatureCollection", "features": [] }};
        let floodExtentsData = {{ "type": "FeatureCollection", "features": [] }};
        let tanaBuffer1kmData = {{ "type": "FeatureCollection", "features": [] }};
        let tanaBuffer3kmData = {{ "type": "FeatureCollection", "features": [] }};
        let farmsData = {{ "type": "FeatureCollection", "features": [] }};
        ...
        function initAllLayers() {{
            ...
            if (typeof garissaCountyData !== 'undefined' && garissaCountyData.features && garissaCountyData.features.length > 0) {{
                overlays["Garissa Boundary"] = L.geoJSON(garissaCountyData, {{
                    style: {{ color: '#00f3ff', fillColor: 'transparent', weight: 2.5, dashArray: '5,5' }},
                    onEachFeature: function(f, l) {{
                        l.bindTooltip("Garissa County", {{ permanent: true, direction: 'center', className: 'custom-tooltip' }});
                    }}
                }});
            }}
            ...
            if (typeof floodExtentsData !== 'undefined' && floodExtentsData.features && floodExtentsData.features.length > 0) {{
                overlays["🌊 Flood Extent Predictions"] = L.geoJSON(floodExtentsData, {{
                    style: {{ color: '#ef4444', fillColor: '#ef4444', fillOpacity: 0.4, weight: 1.5 }},
                    onEachFeature: function(f, l) {{
                        l.bindPopup(buildPopupTable(f.properties.Name || 'Flood Extent', 'FLD-01', 'ACTIVE', f.properties.Risk_Level || 'Extreme', '0.0', '0.95', '🌊'));
                    }}
                }});
            }}
            if (typeof tanaBuffer1kmData !== 'undefined' && tanaBuffer1kmData.features && tanaBuffer1kmData.features.length > 0) {{
                overlays["🛑 Tana River 1km Buffer"] = L.geoJSON(tanaBuffer1kmData, {{
                    style: {{ color: '#f97316', fillColor: '#f97316', fillOpacity: 0.2, weight: 1.5, dashArray: '4,4' }}
                }});
            }}
            if (typeof tanaBuffer3kmData !== 'undefined' && tanaBuffer3kmData.features && tanaBuffer3kmData.features.length > 0) {{
                overlays["🟡 Tana River 3km Buffer"] = L.geoJSON(tanaBuffer3kmData, {{
                    style: {{ color: '#fbbf24', fillColor: '#fbbf24', fillOpacity: 0.1, weight: 1.5, dashArray: '4,4' }}
                }});
            }}
            ...
            if (typeof farmsData !== 'undefined' && farmsData.features && farmsData.features.length > 0) {{
                overlays["🌾 Farms at Risk (LULC)"] = L.geoJSON(farmsData, {{
                    style: {{ color: '#39ff14', fillColor: '#39ff14', fillOpacity: 0.3, weight: 2 }},
                    onEachFeature: function(f, l) {{
                        l.bindPopup(buildPopupTable(f.properties.Name || 'Farm', 'AGR', 'ACTIVE', f.properties.Risk_Level || 'Extreme', '0.0', '0.88', '🌾'));
                        l.on('click', () => notifyParent(f, 'Farms'));
                    }}
                }});
            }}
            ...
            if (typeof cadastralData !== 'undefined' && cadastralData.features && cadastralData.features.length > 0) {{
                overlays["📋 Cadastral Parcels"] = L.geoJSON(cadastralData, {{
                    style: {{ color: '#fbbf24', fillColor: 'transparent', weight: 1.0, dashArray: '2,2' }}
                }});
            }}
        ...
        window.addEventListener('message', (event) => {{
            ...
            }} else if (event.data && event.data.type === 'toggle_layers') {{
                for (let name in overlays) {{
                    map.removeLayer(overlays[name]);
                }}
                if (event.data.layers && Array.isArray(event.data.layers)) {{
                    for (let name of event.data.layers) {{
        ...
        async function loadDataAndInit() {{
            if (window.parent && window.parent.schoolsData) {{
                ...
                cadastralData = window.parent.cadastralData;
                ...
                garissaCountyData = window.parent.garissaCountyData || {{ "type": "FeatureCollection", "features": [] }};
                floodExtentsData = window.parent.floodExtentsData || {{ "type": "FeatureCollection", "features": [] }};
                tanaBuffer1kmData = window.parent.tanaBuffer1kmData || {{ "type": "FeatureCollection", "features": [] }};
                tanaBuffer3kmData = window.parent.tanaBuffer3kmData || {{ "type": "FeatureCollection", "features": [] }};
                farmsData = window.parent.farmsData || {{ "type": "FeatureCollection", "features": [] }};
                ...
                try {{
                    const files = [
                        ...
                        ['cadastralData', 'cadastral_parcels_risk_assessed.geojson'],
                        ...
                        ['baselineFloodData', 'map_1_flood_baseline.geojson'],
                        ['garissaCountyData', 'garissa_county.geojson'],
                        ['floodExtentsData', 'flood_extents.geojson'],
                        ['tanaBuffer1kmData', 'tana_buffer_1km.geojson'],
                        ['tanaBuffer3kmData', 'tana_buffer_3km.geojson'],
                        ['farmsData', 'farms_at_risk.geojson']
                    ];
                    ...
                    cadastralData = window.cadastralData;
                    ...
                    garissaCountyData = window.garissaCountyData || {{ "type": "FeatureCollection", "features": [] }};
                    floodExtentsData = window.floodExtentsData || {{ "type": "FeatureCollection", "features": [] }};
                    tanaBuffer1kmData = window.tanaBuffer1kmData || {{ "type": "FeatureCollection", "features": [] }};
                    tanaBuffer3kmData = window.tanaBuffer3kmData || {{ "type": "FeatureCollection", "features": [] }};
                    farmsData = window.farmsData || {{ "type": "FeatureCollection", "features": [] }};
            ...
            // Signal parent that we are ready to receive postMessages (e.g. for layer toggles)
            if (window.parent) {{
